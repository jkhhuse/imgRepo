...
## javascript 运行时概念
首先是javascript运行时的状态表述，现代javascript引擎实现了类似于下图的模型，包含栈(Stack)、堆(Heap)、队列(Queue)、WebAPI等机制：  
...
依据之前的解释，函数被调用执行，以栈帧的方式压入栈中执行。异步调用进入Web APIs中执行，执行结束后回调函数进行任务队列。当栈为空时，从任务队列中取出第一个函数，压入栈中执行，周而复始，直至队列为空。
...
在日常编程中，经常会遇到`setTimeout(fn, ms)`这样的黑魔法，使用的时候只是简单的理解为提前到某个时间去执行，如果`ms`为0呢？是否是立即执行呢？在理解事件循环机制之前，确实有这样的困扰。
...
这样的执行结果可能与预想的有些出入，产生疑问的地方主要是这个`Promise`执行顺序。

其实，隐藏在背后的的原因就是，事件循环中的`microtask`，js运行时环境中还存在着一个`microtask`队列，
`task`和`MicroTasks`分别对应：
- `task`包含：XHR、setTimeout/setinterval/setImmediate、requestAnimationFrame、UI交互事件等
- `microtasks`包含: Promise、process.nextTick、Object.observe（已废弃）、MutationObserver等
如下图所示：
